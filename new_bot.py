...
#     user_token = 'YOUR-BOT-TOKEN'
    ...
    def set_chat(call):
        ...
        requests.append({})
        requests_count = len(requests) - 1
        # requests_count = update_requests_count('add')
        print('changed request count', requests_count)
    ...
    def tick_button(call, buttons, event):
        ...
        if event == "tick":
            for i in range(len(buttons)):
                btn = buttons[i]
                if call.data == 'WEEK;' + str(i + 1) + ';0':
                    buttons[i]['text'] = '✓ ' + btn['text']
                    buttons[i]['callback'] = 'WEEK;' + str(i + 1) + ';1'
                    curr_weekdays_buttons[i]['text'] = buttons[i]['text']
                    curr_weekdays_buttons[i]['callback'] = buttons[i]['callback']
            print('ticked', curr_weekdays_buttons)

        elif event == "untick":

            for i in range(len(buttons)):
                btn = buttons[i]
                if call.data == 'WEEK;' + str(i + 1) + ';1':
                    print(btn['text'])
                    buttons[i]['text'] = btn['text']
                    buttons[i]['callback'] = 'WEEK;' + str(i + 1) + ';0'
                    curr_weekdays_buttons[i]['text'] = buttons[i]['text']
                    curr_weekdays_buttons[i]['callback'] = buttons[i]['callback']
